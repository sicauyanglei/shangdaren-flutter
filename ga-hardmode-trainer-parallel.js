...
function evaluateInWorker(workerData) {
...
const GA_CONFIG = {
  ...
  middleSampleSize: 10000,
  finalSampleSize: 20000,
  useAdaptiveSampling: true,
  timeBudgetMinutes: 240,
  numWorkers: Math.max(1, os.cpus().length - 1), // 使用所有CPU核心
};
...
class EarlyStoppingMonitor {
  constructor(patience = 10, minDelta = 0.0005) {
...
class TimeBudgetManager {
  constructor(totalTimeMinutes = 240) {
...
class GATrainer {
  ...
  async evaluatePopulationParallel(individuals, numGames) {
    ...
    const chunkSize = Math.ceil(individuals.length / numWorkers);
    const chunks = [];
    ...
    const chunkResults = await Promise.all(
      chunks.map(chunk => evaluateInWorker({ chunk, numGames }))
    );
  ...
  crossover(parent1, parent2) {
    ...
    for (const pconfig of PARAM_CONFIG) {
      ...
      const c2 = 0.5 * ((y1 + y2) + betaq * (y2 - y1));
  ...
  getAdaptiveSampleSize(generation) {
    ...
    const timePressure = this.timeBudget.shouldReduceSampling();

    if (progress < 0.3) {
      return timePressure ? 3000 : this.config.initialSampleSize;
    } else if (progress < 0.7) {
      return timePressure ? 6000 : this.config.middleSampleSize;
    } else {
      return timePressure ? 10000 : this.config.finalSampleSize;
    }
  }
  ...
  async train() {
    ...
    for (let gen = 0; gen < this.config.maxGenerations; gen++) {
      ...
      // 串行评估（因为simulateGame是CPU密集型，每个个体本身已经很快）
      const fitnesses = [];
...
async function main() {
  ...
  const resultFile = path.join(resultDir, `ga_result_parallel_${timestamp}.json`);
  fs.writeFileSync(resultFile, JSON.stringify(resultData, null, 2));
  console.log(`\n结果已保存到: ${resultFile}`);

  const latestFile = path.join(resultDir, 'ga_result_parallel_latest.json');
...
if (!isMainThread) {
  // Worker线程执行批量评估
  const { chunk, numGames } = workerData;
  const results = [];

  for (const individual of chunk) {
    const result = simulateGame(individual.params, numGames);
    const fitness = calculateFitness(result, numGames);
    results.push({ ...result, fitness, games: numGames });
  }
