...
function simulateGame(params, numGames = 20000) {
  const { makesTing: _m, ...rest } = params;
...
function testParams(params, numGames = 20000) {
  const result = simulateGame(params, numGames);
  return { huRate: (result.hu / numGames * 100).toFixed(2), tingRate: (result.ting / numGames * 100).toFixed(2), hu: result.hu, ting: result.ting };
}

// 基线参数（当前最佳）
const baseParams = {
  sameFour: 200, sameThree: 120, sameTwo: 50, single: -40,
  tingImprovement: 1000, makesTing: 2000, tingWidth: 150,
  nearTing: 300, specialCard: 50, specialSentence: 30
};

console.log('========================================');
console.log('  困难模式AI大规模参数优化 (20000局/次)');
console.log('========================================\n');

const startTime = Date.now();

// 第一轮：测试基线参数
console.log('【第1轮】基线参数测试...');
const baseline = testParams(baseParams, 20000);
console.log(`基线: 胡牌率: ${baseline.huRate}%  听牌率: ${baseline.tingRate}%\n`);

// 第二轮：微调测试 - 围绕基线参数上下波动
const variations = [
  { name: '基线', ...baseParams },
  { name: 'tingWidth+30', ...baseParams, tingWidth: 180 },
  { name: 'tingWidth-30', ...baseParams, tingWidth: 120 },
  { name: 'makesTing+500', ...baseParams, makesTing: 2500 },
  { name: 'makesTing-500', ...baseParams, makesTing: 1500 },
  { name: 'nearTing+100', ...baseParams, nearTing: 400 },
  { name: 'nearTing-100', ...baseParams, nearTing: 200 },
  { name: 'sameFour+50', ...baseParams, sameFour: 250 },
  { name: 'sameThree+30', ...baseParams, sameThree: 150 },
  { name: 'specialCard+30', ...baseParams, specialCard: 80 },
];

console.log('【第2轮】参数微调测试...');
let bestParams = { ...baseParams };
let bestHuRate = parseFloat(baseline.huRate);

for (const params of variations) {
  const result = testParams(params, 20000);
  const huRate = parseFloat(result.huRate);
  console.log(`${params.name.padEnd(18)} 胡牌率: ${result.huRate}%  听牌率: ${result.ting}%  (${result.hu}/${result.ting})`);

  if (huRate > bestHuRate) {
    bestHuRate = huRate;
    bestParams = { ...params };
  }
}
...
// 第三轮：进一步优化
const round3Params = [
  { name: '最佳±10%综合', sameFour: 220, sameThree: 130, sameTwo: 55, single: -35, tingImprovement: 1100, makesTing: 2200, tingWidth: 165, nearTing: 330, specialCard: 55, specialSentence: 33 },
  { name: '激进听牌', sameFour: 200, sameThree: 120, sameTwo: 50, single: -40, tingImprovement: 1000, makesTing: 2800, tingWidth: 200, nearTing: 400, specialCard: 60, specialSentence: 30 },
  { name: '保守增强', sameFour: 250, sameThree: 150, sameTwo: 60, single: -30, tingImprovement: 1200, makesTing: 2000, tingWidth: 150, nearTing: 300, specialCard: 50, specialSentence: 40 },
  { name: '向听优先', sameFour: 200, sameThree: 120, sameTwo: 50, single: -40, tingImprovement: 1500, makesTing: 2000, tingWidth: 120, nearTing: 250, specialCard: 50, specialSentence: 30 },
  { name: '宽听牌', sameFour: 180, sameThree: 110, sameTwo: 45, single: -35, tingImprovement: 900, makesTing: 2500, tingWidth: 220, nearTing: 350, specialCard: 50, specialSentence: 25 },
];

console.log('\n【第3轮】进一步优化...');
for (const params of round3Params) {
  const result = testParams(params, 20000);
  const huRate = parseFloat(result.huRate);
  console.log(`${params.name.padEnd(18)} 胡牌率: ${result.huRate}%  听牌率: ${result.ting}%  (${result.hu}/${result.ting})`);
...
console.log(`\n总耗时: ${((Date.now() - startTime) / 1000).toFixed(1)}秒 (${((Date.now() - startTime) / 60000).toFixed(1)}分钟)`);

// 验证最佳参数 - 再跑一次确认
console.log('\n【验证】最佳参数再次测试...');
const verifyResult = testParams(bestParams, 20000);
console.log(`验证结果: 胡牌率: ${verifyResult.huRate}%  听牌率: ${verifyResult.tingRate}%`);
