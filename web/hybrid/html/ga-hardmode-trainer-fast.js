...
class GATrainer {
  ...
  async train() {
    ...
    console.log('  困难模式AI遗传算法训练系统 (快速版-多进程)');
    console.log('='.repeat(60));
    ...
    console.log(`工作进程数: ${this.config.numWorkers}`);
    console.log(`时间预算: ${this.config.timeBudgetMinutes}分钟`);
    console.log('='.repeat(60));

    initWorkerPool(this.config.numWorkers);
    ...
    for (let gen = 0; gen < this.config.maxGenerations; gen++) {
      ...
      const results = await evaluatePopulationParallel(this.population, sampleSize, this.evaluationCache);

      const fitnesses = [];
      for (let i = 0; i < this.population.length; i++) {
        if (results[i] && results[i].hu !== undefined) {
          this.population[i].result = results[i];
          this.population[i].fitness = calculateFitness(results[i], sampleSize);
        } else {
      ...
      if (currentBest.result && currentBest.result.hu !== undefined) {
...
function initWorkerPool(numWorkers) {
  ...
  return new Promise((resolve) => {
    ...
    const assignWork = () => {
      ...
      for (let i = 0; i < population.length; i++) {
        ...
        const cacheKey = getCacheKey(individual.params);
        if (cache.has(cacheKey)) {
          results[i] = cache.get(cacheKey);
          completed++;
          process.stdout.write(`\r评估进度: ${completed}/${population.length}`);
          continue;
        }
        ...
        const messageHandler = (msg) => {
          ...
          if (msg.result) {
            const msgCacheKey = getCacheKey(population[msg.index].params);
            cache.set(msgCacheKey, msg.result);
            if (cache.size > MAX_CACHE_SIZE) {
              const firstKey = cache.keys().next().value;
              cache.delete(firstKey);
            }
          }
          results[msg.index] = msg.result || { hu: 0, ting: 0, fitness: -Infinity };
          completed++;
          process.stdout.write(`\r评估进度: ${completed}/${population.length}`);
