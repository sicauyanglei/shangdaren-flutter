...
function selectAIDiscardHard(player, discardedChars = []) {
  ...
  for (let s = 1; s <= 8; s++) {
    ...
    sentenceCompleteness[s] = {
      ...
      positions,
      isComplete: positions.size === 3,
      isNearlyComplete: positions.size === 2,
  ...
  for (let i = 0; i < hand.length; i++) {
    ...
    if (isCurrentlyTing && tingProbAfter > tingProbCurrent) {
      baseScore += (tingProbAfter - tingProbCurrent) * 700;
    }
...
function shouldAIChiHard(player, card) {
  ...
  for (const opt of chiOptions) {
    ...
    let score = 0;
    if (afterXT < currentXT) score += 600;
    if (afterXT === 0) score += 1000;
    if (opt.sentence === 1 || opt.sentence === 8) score += 150;
    ...
    for (let p = 0; p < 4; p++) {
      ...
      if (p === 0) {
        ...
        if (lastDiscardedCard && lastDiscardPlayer !== p) {
          ...
          if (shouldAIPeng(player, lastDiscardedCard) && pengCount >= 2 && Math.random() < 0.6) {
          ...
          // 吃牌判断
          if (shouldAIChiHard(player, lastDiscardedCard) && Math.random() < 0.5) {
            const s = lastDiscardedCard.sentence;
            let chi1, chi2;
            if (lastDiscardedCard.position === 0) {
              chi1 = ALL_CHARACTERS.find(c => cardMap[c].s === s && cardMap[c].p === 1);
              chi2 = ALL_CHARACTERS.find(c => cardMap[c].s === s && cardMap[c].p === 2);
            } else if (lastDiscardedCard.position === 1) {
            ...
            if (chiCards.length >= 2) {
              ...
              player.melds.push({ type: 'sequence', cards: [...chiCards, lastDiscardedCard], huValue: 0 });
        ...
        if (lastDiscardedCard && lastDiscardPlayer !== p) {
          const pengCount = player.hand.filter(c => c.character === lastDiscardedCard.character).length;
          if (pengCount >= 2 && Math.random() < 0.5) {
            const pengCards = player.hand.filter(c => c.character === lastDiscardedCard.character).slice(0, 2);
            player.hand = player.hand.filter(c => !pengCards.some(pc => pc.id === c.id));
            player.melds.push({ type: 'triplet', cards: [...pengCards, lastDiscardedCard], huValue: 2 });
          ...
          if (shouldAIChiHard(player, lastDiscardedCard) && Math.random() < 0.3) {
            const s = lastDiscardedCard.sentence;
            let chi1, chi2;
            if (lastDiscardedCard.position === 0) {
              chi1 = ALL_CHARACTERS.find(c => cardMap[c].s === s && cardMap[c].p === 1);
...
async function runHuRateTest(numGames = 500) {
  console.log('========================================');
  console.log('  困难模式AI胡牌率测试 v5 (修复版)');
  console.log('========================================');
