...
function simulateGame(params, numGames) {
  ...
  for (let g = 0; g < numGames; g++) {
    ...
    while (turn < 300 && deck.length > 0 && !gameEnded) {
      ...
      if (g % 10000 === 0 && turn === 1) {
        process.stdout.write(`\r进度: ${g}/${numGames}`);
      }
    }
  }
...
function testParams(name, params, numGames) {
  const result = simulateGame(params, numGames);
  const huRate = (result.hu / numGames * 100).toFixed(2);
  const tingRate = (result.ting / numGames * 100).toFixed(2);
  return { name, huRate, tingRate, hu: result.hu, ting: result.ting };
}

// 测试参数组合
const paramSets = [
  { name: '基线(200,120,50)', sameFour: 200, sameThree: 120, sameTwo: 50, single: -40, tingImprovement: 1000, makesTing: 2000, tingWidth: 150, nearTing: 300, specialCard: 50, specialSentence: 30 },
  { name: '激进听牌', sameFour: 200, sameThree: 120, sameTwo: 50, single: -40, tingImprovement: 1000, makesTing: 2500, tingWidth: 200, nearTing: 400, specialCard: 50, specialSentence: 30 },
  { name: '保守增强', sameFour: 250, sameThree: 150, sameTwo: 60, single: -30, tingImprovement: 1200, makesTing: 2000, tingWidth: 150, nearTing: 300, specialCard: 60, specialSentence: 40 },
  ...
  { name: '综合A', sameFour: 220, sameThree: 130, sameTwo: 55, single: -35, tingImprovement: 1100, makesTing: 2200, tingWidth: 165, nearTing: 330, specialCard: 55, specialSentence: 33 },
  { name: '综合B', sameFour: 240, sameThree: 140, sameTwo: 55, single: -30, tingImprovement: 1100, makesTing: 2100, tingWidth: 160, nearTing: 320, specialCard: 55, specialSentence: 35 },
  { name: '极端激进', sameFour: 180, sameThree: 100, sameTwo: 40, single: -30, tingImprovement: 1200, makesTing: 3000, tingWidth: 250, nearTing: 450, specialCard: 60, specialSentence: 30 },
];

const NUM_GAMES = 200000;

console.log('========================================');
console.log(`  困难模式AI大规模优化 (${NUM_GAMES}局/次)`);
console.log('========================================\n');

const startTime = Date.now();

console.log(`测试 ${paramSets.length} 组参数，每组 ${NUM_GAMES} 局\n`);
...
for (let i = 0; i < paramSets.length; i++) {
  ...
  process.stdout.write(`\n[${i+1}/${paramSets.length}] 测试: ${params.name}... `);
  const result = testParams(params.name, params, NUM_GAMES);
  results.push(result);
  console.log(`\n  胡牌率: ${result.huRate}%  听牌率: ${result.tingRate}%`);
}

console.log('\n' + '='.repeat(60));
console.log('  测试结果汇总');
console.log('='.repeat(60));
console.log(`${'参数'.padEnd(20)} ${'胡牌率'.padStart(10)} ${'听牌率'.padStart(10)}`);
console.log('-'.repeat(60));

results.sort((a, b) => parseFloat(b.huRate) - parseFloat(a.huRate));
let best = results[0];
for (const r of results) {
  const marker = r === best ? ' ★' : '';
  console.log(`${r.name.padEnd(20)} ${r.huRate.padStart(10)}% ${r.tingRate.padStart(10)}%${marker}`);
}

console.log('\n' + '='.repeat(60));
console.log('最佳参数配置:');
console.log('-'.repeat(60));
const bestParams = paramSets.find(p => p.name === best.name);
console.log(JSON.stringify(bestParams, null, 2));
console.log('\n' + '='.repeat(60));

const elapsed = Date.now() - startTime;
console.log(`\n总耗时: ${(elapsed / 60000).toFixed(1)} 分钟`);
