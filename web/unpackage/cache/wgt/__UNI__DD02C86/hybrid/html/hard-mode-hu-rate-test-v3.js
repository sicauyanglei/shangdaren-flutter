...
function selectAIDiscardHard(player, discardedChars = []) {
  ...
  for (let i = 0; i < hand.length; i++) {
    ...
    if (sameCharCount >= 4) {
      baseScore += 150; // 四张相同，保留形成招
    } else if (sameCharCount === 3) {
      baseScore += 100; // 三张相同，保留形成坎
    } else if (sameCharCount === 2) {
      baseScore += 40; // 两张相同，保留形成对子
    } else {
      baseScore -= 30; // 单张优先打出
    }
    ...
    // 10. 形成招/坎的潜力评估
    // 如果手牌中有3张相同的，这张牌打出后会失去形成坎的机会
    if (sameCharCount === 3) {
      // 检查打出后是否还能形成坎
      const tempCounts = {...counts};
      tempCounts[card.character]--;
      // 不做额外惩罚，因为已经有3张了
    }
